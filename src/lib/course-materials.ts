...
/**
 * קישורי מצגת/חוברת לפתיחה ישירה.
 * עדיפות: קישור מהגדרות הקורס → אותם קבצי public כמו «שלח חוברת» / «מצגת קורס 44» בפעולות מהירות.
 */
export function resolveCourseMaterialOpenUrls(opts: {
  ...
  const fromCatalogPresentation = (opts.presentationUrl || "").trim()
  const fromCatalogBooklet = (opts.bookletUrl || "").trim()

  // אותם קישורים כמו בכפתורים המהירים באדמין (שלח חוברת / מצגת 44 PDF)
  const adminBooklet = courseMaterialUrl("booklet44Pdf", opts.baseUrl)
  const adminPresentation = courseMaterialUrl("presentation44Pdf", opts.baseUrl)

  return {
    presentationUrl: fromCatalogPresentation || adminPresentation || null,
    bookletUrl: fromCatalogBooklet || adminBooklet || null,
  }
}
